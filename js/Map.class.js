class Map {
  ...
  load(data) {
    var newLayer = []
    for (var l = 1; l < data.layers.length; l++) {
      var newArray = listToMatrix(data.layers[l].data, data.layers[l].width)
      makeCoordinates(newArray, data.tilesets[0].columns)
      newLayer.push(newArray)
    }


    for (var i = 0; i < data.layers[0].objects.length; i++) {
      this.collisionData.push({ type: data.layers[0].objects[i].index, posy: data.layers[0].objects[i].y, posx: data.layers[0].objects[i].x })

    }
    console.log(this.collisionData)
    /* for(){
      this.collisionData.push({ type: brush.data, posy: posy, posx: posx })
    } */
    console.log(newLayer)
    //   this.width = this.intW * this.gridWidth
    //  this.height = this.intH * this.gridHeight
    this.width = data.width * data.displayWidth
    this.height = data.height * data.displayHeight
    this.gridWidth = data.displayWidth
    this.gridHeight = data.displayHeight
    this.intW = data.width
    this.intH = data.height
    this.nLayers = newLayer.length
