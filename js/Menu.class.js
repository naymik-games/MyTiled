const Menu = {
  ...
  'map': {
    ...
    'tiled': (arguments = null) => {
      if (!arguments) {
        const conf = {
          'name': 'text'
        }
        return Menu.showWindow(conf, Menu.map.tiled)
      }
      exportTiled(arguments[0])
    },
  ...
  'other': {
    'default': (arugments = null) => {
      if (!arguments) {
        ...
        return Menu.showWindow(conf, Menu.other.default)
      }
      //createNewCustomBrush(...arguments.map(x => parseInt(x) || 0))
    }
  }
